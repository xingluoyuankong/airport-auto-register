...
resp=requests.get("https://graph.microsoft.com/v1.0/me/messages?$top=3&$select=subject,from,receivedDateTime,bodyPreview,body&$orderby=receivedDateTime desc",
    headers={"Authorization":"Bearer "+at},timeout=10)
for m in resp.json().get("value",[]):
    subj=m.get("subject","")
    frm=m.get("from",{}).get("emailAddress",{}).get("address","")
    if "glados" in str(frm).lower() or "glados" in str(subj).lower():
        body=m.get("body",{}).get("content","")
        preview=m.get("bodyPreview","") or ""
        full=body+preview
        print(f"Subject: {subj}")
        codes=re.findall(r"\b\d{4,8}\b", full)
