...
with sync_playwright() as p:
    ...
    pg=ctx.new_page()
    pg.goto("https://www.v2ny.com/#/register",wait_until="domcontentloaded",timeout=60000)
    time.sleep(5)
    print("=== v2ny ===")
    ...
    for el in pg.query_selector_all("input"):
        a=el.evaluate("el=>({t:el.type,n:el.name,p:el.placeholder})")
    ...
    at=pg.evaluate("""()=>{
        ...
        while(w.nextNode()){
            ...
            if(/@(?:qq|163|gmail|outlook|yahoo|hotmail|126|sina|foxmail|yeah|icloud)/i.test(t)){
                ...
                r.push({tag:p.tagName,cls:p.className,text:t.trim().substring(0,40)});
            }
        }
        return JSON.stringify(r,null,2);
    }""")
    ...
    pg.screenshot(path="v2ny_deep.png")
    b.close()
