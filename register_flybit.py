# -*- coding: utf-8 -*-
"""FLYBIT 正确版 — A型表单(Naive UI下拉框)
面板: flybit.vip  免费: 2GB/24h  系统: V2Board+NaiveUI
关键: 邮箱前缀+Naive UI的n-select下拉选后缀
"""
...
def run(email_addr,reg_pass):
    # 拆分邮箱: mx40f8e7ef94@outlook.com -> prefix=mx40f8e7ef94, suffix=@outlook.com
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # 2. fill email PREFIX (不带@后缀!)
            print(f"[2] fill prefix: {prefix}")
            pg.fill('input[type="email"]',prefix)
            
            # 3. select domain suffix (Naive UI n-select)
            print(f"[3] select suffix: {suffix}")
            # 点击下拉框触发弹窗
            pg.click(".n-base-selection-label")
            time.sleep(1)
            # 点击对应选项 (text=避免CSS @转义问题)
            ...
            # 9. extract subscribe via VUE_NAIVE_ACCESS_TOKEN
            ...
            if raw:
                ...
                sub_url=""
                for u in urls:
                    if "subscribe" in u.lower() or "sub" in u.lower():
                        sub_url=u
                        print(f"SUB_URL: {u}")
                
                result={"airport":"FLYBIT","panel":"flybit.vip","system":"V2Board+NaiveUI",
                        "email":email_addr,"password":reg_pass,"subscribe_url":sub_url or resp}
                os.makedirs("register_results",exist_ok=True)
                nm=prefix
                with open(f"register_results/flybit_{nm}.json","w",encoding="utf-8") as f:
                    json.dump(result,f,ensure_ascii=False,indent=2)
                print(f"saved register_results/flybit_{nm}.json")
                return sub_url or resp
            else:
                print("NO TOKEN - reg failed")
                pg.screenshot(path="flybit_fail.png")
                return None
