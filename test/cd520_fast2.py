...
def wait_code(email, timeout=30):
    ...
    while time.time() < dl:
        try:
            ...
            resp = req.get("https://graph.microsoft.com/v1.0/me/messages?$top=15&$orderby=receivedDateTime desc&$select=id,subject,from,bodyPreview",
            ...
            for msg in resp.json().get("value", []):
                ...
                seen.add(mid)
                # CD520 uses cd520.xyz / cd1314.xyz
                fr = (msg.get('from') or {}).get('emailAddress') or {}
                fr_addr = fr.get('address','')
                subject = msg.get('subject','') or ''
                if 'cd520' not in fr_addr.lower() and 'cd1314' not in fr_addr.lower():
                    continue  # skip non-cd520 mail
                combined = f"{subject} {msg.get('bodyPreview','')}"
                for pat in [r"\b(\d{6})\b", r"(?:验证码|激活码)\D{0,30}?(\d{4,8})"]:
                    ...
                    if m and len(m.group(1)) >= 5 and m.group(1) not in ("000000","111111","222222","999999","123456"):
                        ...
                        print(f"  [{elapsed}s] from={fr_addr} code={code}", flush=True)
                        return code
            time.sleep(2)
        except: time.sleep(2)
    return None
...
with sync_playwright() as p:
    ...
    try:
        ...
        page.screenshot(path=os.path.join(os.path.dirname(__file__), "cd520_res.png"))
        
        if "/login" in page.url or "登入" in body:
            print("login...", flush=True)
            page.fill('input[placeholder*="邮箱"], input[type="email"]', EMAIL)
            time.sleep(0.3)
            page.fill('input[type="password"]', PASS)
            time.sleep(0.3)
            for t in ["登录","登入","Login"]:
                try: page.click(f'button:has-text("{t}")',timeout=2000); time.sleep(4); break
                except: continue
            body = page.evaluate("()=>document.body.innerText")
            print(f"after:{page.url}\n{body[:300]}", flush=True)
        ...
        print(f"text={m.group(0) if m else 'NONE'}", flush=True)
        time.sleep(10)
