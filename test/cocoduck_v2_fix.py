...
def wait_code(email, timeout=120):
    ...
    while time.time() < dl:
        try:
            ...
            print(f"  [Graph] 异常: {e}", flush=True); time.sleep(5)
    return None, "timeout"
...
with sync_playwright() as p:
    ...
    try:
        print("\n[1] 打开注册页...", flush=True)
        page.goto("https://cocoduck.cc/#/register", wait_until="networkidle", timeout=45000)
        time.sleep(4)
        body = page.evaluate("() => document.body.innerText")
        print(f"  页面(500):\n{body[:500]}", flush=True)
        
        # 检查Turnstile
        cf = page.query_selector('iframe[src*="challenges.cloudflare.com"], iframe[src*="turnstile"]')
        if cf:
            print("  [Turnstile] 等待60s...", flush=True)
            dl = time.time() + 60
            while time.time() < dl:
        ...
        # COCODUCK可能是完整邮箱input
        email_inputs = page.query_selector_all('input[type="email"], input[placeholder*="邮箱"]')
        if email_inputs:
            email_inputs[0].fill(EMAIL)
            print(f"  ✅ 填完整邮箱: {EMAIL}", flush=True)
        else:
            # A型前缀+后缀
            prefix, suffix = EMAIL.split("@", 1); suffix = "@"+suffix
            print(f"  A型: prefix={prefix} suffix={suffix}", flush=True)
        ...
        print(f"  密码: 已填{len(pws)}个", flush=True)
        
        page.screenshot(path=os.path.join(os.path.dirname(__file__), "cocoduck_v2_filled.png"))
        ...
        page.screenshot(path=os.path.join(os.path.dirname(__file__), "cocoduck_v2_after_register.png"))
        
        # 登录（如果需要）
        if "/login" in page.url or "登录" in body2:
            print("  → 登录...", flush=True)
            ei = page.query_selector_all('input[type="email"], input[placeholder*="邮箱"]')
            if ei: ei[0].fill(EMAIL); time.sleep(0.3)
        ...
        if raw:
            ...
            if sub_url:
                ...
                nm = EMAIL.split("@")[0]
                with open(os.path.join(os.path.dirname(__file__), f"cocoduck_{nm}.json"), "w", encoding="utf-8") as f:
