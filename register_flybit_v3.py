...
# -*- coding: utf-8 -*-
"""
FLYBIT 注册脚本 V3 — 专业验证码获取 + 截图审计
面板: flybit.vip  |  免费: 2GB测试体验  |  系统: NaiveUI(V2Board)
用法: python register_flybit_v3.py [email] [password]
"""
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.screenshot(path=os.path.join(
                SCREENSHOT_DIR, "flybit_v3_step1_register.png"))
            ...
            page.screenshot(path=os.path.join(
                SCREENSHOT_DIR, "flybit_v3_step2_filled.png"))

            # ──────── Step 3: 发送验证码 ────────
            print("\n[3/6] 发送验证码...", flush=True)
            page.screenshot(path=os.path.join(
                SCREENSHOT_DIR, "flybit_v3_step3a_before_send.png"))
            page.click('button:has-text("发送")')
            time.sleep(1)
            page.screenshot(path=os.path.join(
                SCREENSHOT_DIR, "flybit_v3_step3b_after_send.png"))
            ...
            # ──────── Step 4: 收验证码（专业） ────────
            print("\n[4/6] 等待验证码 (发件人: FlyBit)...", flush=True)
            code = fetcher.fetch(
                sender_keywords=["FlyBit", "flybit", "FLYBIT", "flybit.vip"],
                timeout=90,
                retries=3)
            if not code:
                print("  ❌ 验证码获取失败", flush=True)
                page.screenshot(path=os.path.join(
                    SCREENSHOT_DIR, "flybit_v3_step4_nocode.png"))
                browser.close()
                return None
            ...
            time.sleep(0.3)
            page.screenshot(path=os.path.join(
                SCREENSHOT_DIR, "flybit_v3_step5a_code_filled.png"))
            ...
            page.screenshot(path=os.path.join(
                SCREENSHOT_DIR, "flybit_v3_step5b_after_register.png"))
            ...
            if "/login" in cur_url or "登入" in body:
                ...
                page.screenshot(path=os.path.join(
                    SCREENSHOT_DIR, "flybit_v3_step5c_after_login.png"))
            ...
            try:
                page.screenshot(path=os.path.join(
                    SCREENSHOT_DIR, "flybit_v3_error.png"))
            except:
                pass
            return None
        finally:
...
if __name__ == "__main__":
    EMAIL = sys.argv[1] if len(sys.argv) > 1 else "adavisvgkxmdpac1af9rcn@outlook.com"
